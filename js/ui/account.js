...
(function (root) {
  ...
  function render(state) {
    ...
    el.innerHTML = C.pageTitle('◉', 'Account', 'Your ID, subscription and verification')
      ...
      + '<div class="controls" style="margin-top:12px">' + (premium ? '' : '<button type="button" class="btn primary" data-act="upgrade">Upgrade — $' + cfg.priceUsd + '/month</button>') + '<button type="button" class="btn" data-act="accuracy">Accuracy</button><button type="button" class="btn" data-act="signal">Matches Signal</button>' + (A.isAdmin(p, cfg) ? '<button type="button" class="btn" data-act="admin">Admin console</button><button type="button" class="btn ghost" data-act="settings">Analysis settings</button>' : '') + '<button type="button" class="btn ghost" data-act="logout">Log out</button></div></div>'
    ...
    ['accuracy', 'signal', 'admin', 'settings'].forEach(pg => { const b = el.querySelector('[data-act="' + pg + '"]'); if (b) b.addEventListener('click', () => act.navigate(pg)); });
