...
(function (root) {
  ...
  function render() {
    if (!el) return;
    const mailNote = data.mailConfigured === false ? '<div class="banner" style="margin-bottom:12px"><div class="banner-title">Email sending is not configured yet</div><div class="banner-body">Add the secrets GMAIL_USER and GMAIL_APP_PASSWORD (or RESEND_API_KEY) in Supabase → Edge Functions → Secrets. Until then, approvals show the activation code here so you can send it by WhatsApp.</div></div>' : '';
    ...
    const userRows = data.users.map(u => {
      ...
      return ['<span class="mono">' + C.esc(u.account_id) + '</span>' + (u.verified && premium ? ' <span class="tick small-tick">✓</span>' : ''), C.esc(u.email) + (u.email_verified ? '' : ' <span class="warn-t small">unverified</span>'), C.esc(u.phone || '—'), premium ? '<span class="ok-t">PREMIUM</span> until ' + C.esc(new Date(u.premium_until).toLocaleDateString()) : '<span class="muted">free</span>',
        '<div class="row"><button type="button" class="btn small-btn" data-verify="' + C.esc(u.email) + '">Verify now</button><button type="button" class="btn ghost small-btn" data-code="' + C.esc(u.email) + '">Send code</button>' + (premium ? '<button type="button" class="btn danger small-btn" data-revoke="' + C.esc(u.email) + '">Revoke</button>' : '') + '</div>'];
    });
    ...
    el.innerHTML = C.pageTitle('⚑', 'Admin', 'Claims, verification, users, receiving details') + mailNote + codeNotice()
      ...
      + '<div class="card"><div class="card-head"><h3>Verify any user</h3><div class="sub">works whether or not they have paid — instant PREMIUM + tick, or email them a code</div></div>'
      ...
      + '<div class="controls"><button type="submit" class="btn primary">Verify now</button><button type="button" class="btn" data-act="send-code">Send activation code</button></div></form></div>'
