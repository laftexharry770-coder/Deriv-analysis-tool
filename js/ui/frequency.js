...
(function (root) {
  ...
  function render(state) {
    ...
    const sel = '<div class="selects"><select data-set="symbol" aria-label="Market">' + markets.map(x => '<option value="' + C.esc(x.symbol) + '"' + (x.symbol === state.active ? ' selected' : '') + '>' + C.esc(x.name) + '</option>').join('') + '</select>'
      + '<select data-set="window" aria-label="Window">' + [100, 200, 250, 300, 500].map(w => '<option value="' + w + '"' + (Number(state.settings.window) === w ? ' selected' : '') + '>' + w + ' ticks</option>').join('') + '</select>'
    ...
    el.innerHTML = C.pageTitle('▥', 'Frequency graph', 'Appearance rate per digit against the 10% baseline') + '<div class="card"><div class="card-head"><div><h3>' + C.esc(m ? m.name : 'No market') + '</h3><div class="sub">appearance rate over the selected window</div></div></div>' + sel + '<div id="fq-body">' + bodyHtml(state) + '</div></div>';
  ...
  // per-tick partial update: the chart body (chips, count, bars, circles, meta) is rebuilt; the selectors are left alone
  function renderLive(state) {
    if (!el) return;
    const body = el.querySelector('#fq-body');
    if (!body) { render(state); return; }
    const html = bodyHtml(state); if (body.innerHTML !== html) body.innerHTML = html;
  }
